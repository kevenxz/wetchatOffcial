...
- 2026-04-19：已重构 `plan_article_angle` 的通用 fallback 结构，默认不再产出“先给结论 / 发生变化的核心原因”这类固定公众号标题，而是根据主题本身生成内容驱动的公众号章节标题。
- 2026-04-19：已收紧模型侧蓝图规划约束，prompt 现在明确要求“公众号编辑式结构”，并在进入写作前把模型返回的章节数标准化到 4-6 段。
- 2026-04-19：已放宽 `compose_draft` 对蓝图 H2 标题的硬保留约束，写作阶段现在保持章节意图和顺序，但允许把标题润色成更像公众号成稿的表达。
- 2026-04-19：已继续收紧 `compose_draft` 的成稿约束，prompt 现在会明确要求输出可发布标题、简洁摘要，以及首个 H2 之前的开头钩子段。
- 2026-04-19：已再次深化 `compose_draft`，起草输出现在会补齐 `alt_titles`，并把 `summary` 与候选标题一起回写到 `generated_article`，后续发布链路不再只拿到一个单标题成稿。
- 2026-04-19：已深化 `review_article_draft` 的 fallback 规则；当草稿开头还是“本文将从……”这类路线图式空泛开场时，会直接打回并给出开头钩子重写指引，避免无模型评审时漏过低信息量首段。
- 2026-04-20：已继续收紧 `compose_draft` 的成稿约束，prompt 现在会明确要求章节之间有自然过渡，并要求每个 H2 小节至少展开成 2-3 段，减少“每节只写一句”的拼接感。
- 2026-04-20：已再次深化 `review_article_draft` 的 fallback 规则；当小节正文过薄、每节几乎只有一句话时，也会直接打回并附带段落密度与过渡补写指引，避免结构存在但成稿感不足的文章漏过。
- 2026-04-19：已在 `generate_visual_assets` 增加正文插图判定；非封面图现在会按顺序插入正文 Markdown 占位符，如 `[插图1]`，让文章内容本身携带图片插入位。
- 2026-04-19：已验证微信草稿推送阶段会上传正文插图，并在 `draft/add` 前把 `[插图N]` 占位符替换成微信托管图片地址，打通“图片规划 -> 正文插图 -> 微信渲染”整条链路。
- 2026-04-19：已将正文插图放置从简单顺序插入升级为按章节语义放置；`infographic` 会优先贴近数据/证据段落，场景类插图会优先贴近案例/公司段落，同时 `generated_article.illustrations` 会按最终正文占位顺序重排，保证微信替换时图片和段落位置一致。
- 2026-04-20：已修复 `generate_visual_assets` 在当前 Gemini 兼容提供方下“图片未生成”的真实根因。该提供方返回的是 `b64_json` 而不是远程 `url`，现在视觉链路会把 base64 图片字节落盘到 `artifacts/generated_images`，并正确回写到文章图片字段，当前环境下已能真实生成封面图文件。
